...
pub fn parseDescr(descr: []const u8) ParseDescrError!DescrData {
    if (descr.len < 3) {
    ...
    const element_type: ElementType = switch (type_char) {
        ...
        'b' => blk: {
            ...
            if (descr.len != 3 or descr[2] != '1') {
                return ParseDescrError.InvalidValue;
            }
            break :blk .Bool;
        },
        ...
        'i', 'u' => blk: {
            ...
            if (size_slice.len != 1) {
                return ParseDescrError.UnsupportedType;
            }
            ...
            if (type_char == 'i') {
                switch (size_char) {
                    ...
                    '4' => break :blk .Int32,
                    '8' => break :blk .Int64,
                    else => return ParseDescrError.UnsupportedType,
                }
            } else { // type_char == 'u'
                switch (size_char) {
                    '1' => break :blk .UInt8,
...
test "parse bool dtype" {
    const result = try parseDescr("|b1");
...
test "parse signed integer dtypes" {
    // Int8 (|i1)
    const i8_result = try parseDescr("|i1");
    ...
    // Int16 (<i2, >i2)
    const i16_little = try parseDescr("<i2");
    ...
    const i16_big = try parseDescr(">i2");
    ...
    // Int32 (<i4, >i4)
    const i32_little = try parseDescr("<i4");
    ...
    const i32_big = try parseDescr(">i4");
    ...
    // Int64 (<i8, >i8)
    const i64_little = try parseDescr("<i8");
    ...
    const i64_big = try parseDescr(">i8");
...
test "parse unsigned integer dtypes" {
    // UInt8 (|u1)
    const u8_result = try parseDescr("|u1");
    ...
    // UInt16 (<u2, >u2)
    const u16_little = try parseDescr("<u2");
    ...
    const u16_big = try parseDescr(">u2");
    ...
    // UInt32 (<u4, >u4)
    const u32_little = try parseDescr("<u4");
    ...
    const u32_big = try parseDescr(">u4");
    ...
    // UInt64 (<u8, >u8)
    const u64_little = try parseDescr("<u8");
    ...
    const u64_big = try parseDescr(">u8");
...
test "parse floating point dtypes" {
    // Float32 (<f4, >f4)
    const f32_little = try parseDescr("<f4");
    ...
    const f32_big = try parseDescr(">f4");
    ...
    // Float64 (<f8, >f8)
    const f64_little = try parseDescr("<f8");
    ...
    const f64_big = try parseDescr(">f8");
    ...
    // Float128 (<f16, >f16)
    const f128_little = try parseDescr("<f16");
    ...
    const f128_big = try parseDescr(">f16");
...
test "parse with native endianness" {
    const i32_native = try parseDescr("=i4");
    ...
    const u64_native = try parseDescr("=u8");
    ...
    const f64_native = try parseDescr("=f8");
...
test "error on too short descr" {
    try std.testing.expectError(ParseDescrError.TooShort, parseDescr(""));
    try std.testing.expectError(ParseDescrError.TooShort, parseDescr("<"));
}

test "error on invalid endianness" {
    try std.testing.expectError(ParseDescrError.InvalidEndianness, parseDescr("@i4"));
    try std.testing.expectError(ParseDescrError.InvalidEndianness, parseDescr("!f8"));
}

test "error on invalid endianness for bool" {
    try std.testing.expectError(ParseDescrError.InvalidEndianness, parseDescr("<b1"));
    try std.testing.expectError(ParseDescrError.InvalidEndianness, parseDescr(">b1"));
    try std.testing.expectError(ParseDescrError.InvalidEndianness, parseDescr("=b1"));
}

test "error on invalid endianness for i1/u1" {
    try std.testing.expectError(ParseDescrError.InvalidEndianness, parseDescr("<i1"));
    try std.testing.expectError(ParseDescrError.InvalidEndianness, parseDescr(">u1"));
    try std.testing.expectError(ParseDescrError.InvalidEndianness, parseDescr("=i1"));
}

test "error on invalid endianness for multi-byte integers" {
    try std.testing.expectError(ParseDescrError.InvalidEndianness, parseDescr("|i2"));
    try std.testing.expectError(ParseDescrError.InvalidEndianness, parseDescr("|u4"));
    try std.testing.expectError(ParseDescrError.InvalidEndianness, parseDescr("|i8"));
}

test "error on invalid endianness for floats" {
    try std.testing.expectError(ParseDescrError.InvalidEndianness, parseDescr("|f4"));
    try std.testing.expectError(ParseDescrError.InvalidEndianness, parseDescr("|f8"));
    try std.testing.expectError(ParseDescrError.InvalidEndianness, parseDescr("|f16"));
}

test "error on invalid bool size" {
    try std.testing.expectError(ParseDescrError.InvalidValue, parseDescr("|b2"));
    try std.testing.expectError(ParseDescrError.InvalidValue, parseDescr("|b4"));
    try std.testing.expectError(ParseDescrError.TooShort, parseDescr("|b"));
}

test "error on unsupported integer sizes" {
    try std.testing.expectError(ParseDescrError.UnsupportedType, parseDescr("<i3"));
    try std.testing.expectError(ParseDescrError.UnsupportedType, parseDescr(">u5"));
    try std.testing.expectError(ParseDescrError.UnsupportedType, parseDescr("=i16"));
}

test "error on unsupported float sizes" {
    try std.testing.expectError(ParseDescrError.UnsupportedType, parseDescr("<f2"));
    try std.testing.expectError(ParseDescrError.UnsupportedType, parseDescr(">f12"));
    try std.testing.expectError(ParseDescrError.UnsupportedType, parseDescr("=f32"));
}

test "error on unsupported type characters" {
    try std.testing.expectError(ParseDescrError.UnsupportedType, parseDescr("<c8"));
    try std.testing.expectError(ParseDescrError.UnsupportedType, parseDescr(">S10"));
    try std.testing.expectError(ParseDescrError.UnsupportedType, parseDescr("=U5"));
    try std.testing.expectError(ParseDescrError.TooShort, parseDescr("|O"));
    try std.testing.expectError(ParseDescrError.UnsupportedType, parseDescr("<M8"));
}

test "error on invalid integer descr length" {
    try std.testing.expectError(ParseDescrError.TooShort, parseDescr("<i"));
    try std.testing.expectError(ParseDescrError.TooShort, parseDescr(">u"));
    try std.testing.expectError(ParseDescrError.UnsupportedType, parseDescr("<i44"));
}

test "error on invalid float descr length" {
    try std.testing.expectError(ParseDescrError.TooShort, parseDescr("<f"));
    try std.testing.expectError(ParseDescrError.UnsupportedType, parseDescr(">f123"));
}
